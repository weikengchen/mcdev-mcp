...
if sys.version_info < (3, 7):
    raise OSError("Python verson must be 3.7 or above.")
        ...
        if not results:
            ...
            for flag in [winreg.KEY_WOW64_64KEY, winreg.KEY_WOW64_32KEY]:
                try:
                    ...
                    k = winreg.OpenKey(
                        winreg.HKEY_LOCAL_MACHINE,
                        r"Software\JavaSoft\Java Development Kit\%s" % version,
                        0,
                        winreg.KEY_READ | flag,
                    )
...
def get_version_manifest(target_version: str, quiet) -> None:
    ...
    with open(path_to_json) as f:
        ...
        for version in versions:
            if (
                version.get("id")
                and version.get("id") == target_version
                and version.get("url")
            ):
...
def get_version_jar(target_version: str, side: SideType, quiet) -> None:
    ...
    with open(path_to_json) as f:
        jsn = json.load(f)
        if (
            not jsn.get("downloads")
            or not jsn.get("downloads").get(side)
            or not jsn.get("downloads").get(side).get("url")
        ):
            raise RuntimeError("Could not download jar, missing fields")
        download_file(
            jsn.get("downloads").get(side).get("url"), target_side_path, quiet
        )
        ...
        if side == SERVER:
            ...
            with zipfile.ZipFile(target_side_path, mode="r") as z:
                ...
                if content is not None:
                    ...
                    try:
                        new_jar_path = z.extract(
                            Path("META-INF", "versions", path), new_jar_path
                        )
                    except Exception as e:
                        raise RuntimeError(f"Could not extract to {new_jar_path}: {e}")
                    if not (PATH_TO_ROOT_DIR / new_jar_path).exists():
                        raise RuntimeError(
                            f"New {side} jar could not be extracted from archive at {new_jar_path}."
                        )
...
def get_mappings(version: str, side: SideType, quiet) -> bool:
    ...
    with open(path_to_json) as f:
        ...
        if side == CLIENT:
            if "client_mappings" not in url or "url" not in url["client_mappings"]:
                if not quiet:
                    print(
                        f"No client mappings found for {version} (unobfuscated dev snapshot)"
                    )
                return False
            ...
            if "server_mappings" not in url or "url" not in url["server_mappings"]:
                if not quiet:
                    print(
                        f"No server mappings found for {version} (unobfuscated dev snapshot)"
                    )
                return False
        ...
        download_file(
            ...
            (
                ...
                / "mappings"
                / version
                / f"{'client' if side == CLIENT else 'server'}.txt"
            ),
            quiet,
        )
        return True
...
def remap(version: str, side: SideType, quiet, lib_dir: Path = None) -> None:
    if not quiet:
    ...
    if not path.exists() or not path.is_file():
        ...
        if path_temp.is_file():
            ...
            r = (
                input(
                    "Error: defaulting to client.jar from your local Minecraft folder. Continue? (y/n)"
                )
                or "y"
            )
            if r != "y":
    ...
    if lib_dir is None:
        lib_dir = PATH_TO_ROOT_DIR / "lib"
    special_source_path = lib_dir / f"SpecialSource-{SPECIAL_SOURCE_VERSION}.jar"
...
def decompile_fern_flower(
    decompiled_version: str,
    version: str,
    side: SideType,
    quiet,
    force,
    ...
    if lib_dir is None:
        lib_dir = PATH_TO_ROOT_DIR / "lib"
    fernflower = lib_dir / "fernflower.jar"
    ...
    if not quiet:
        print(
            f"Done in %.1fs (file was decompressed in {decompiled_version}/{side})" % t
        )
        # TODO: Automate choice if auto mode is enabled
...
def decompile_cfr(
    decompiled_version: str, version: str, side: SideType, quiet, lib_dir: Path = None
) -> None:
    if not quiet:
    ...
    if lib_dir is None:
        lib_dir = PATH_TO_ROOT_DIR / "lib"
    cfr = lib_dir / f"cfr-{CFR_VERSION}.jar"
...
def remap_file_path(path: str) -> str:
    ...
    return (
        "L" + "/".join(path.split(".")) + ";"
        if path not in remap_primitives
        else remap_primitives[path]
    )
    ...
    with open(version_side_path, "r") as inputFile:
        ...
        for line in inputFile.readlines():
            ...
            if not line.startswith("    "):
                ...
                file_name[remap_file_path(deobf_name)] = (
                    obf_name  # save it to compare to put the Lb
                )

    with (
        open(version_side_path, "r") as inputFile,
        open(
            PATH_TO_ROOT_DIR / "mappings" / version / f"{side}.tsrg", "w+"
        ) as outputFile,
    ):
        for line in inputFile.readlines():
            if line.startswith("#"):  # comment at the top, could be stripped
            ...
            if line.startswith("    "):
                ...
                method_type, method_name = deobf_name.split(
                    " "
                )  # split the `<methodType> <methodName>`
                method_type = method_type.split(
                    ":"
                )[
                    -1
                ]  # get rid of the line numbers at the beginning for functions eg: `14:32:void`-> `void`
                if (
                    "(" in method_name and ")" in method_name
                ):  # detect a function function
                    variables = method_name.split("(")[-1].split(")")[
                        0
                    ]  # get rid of the function name and parenthesis
                    function_name = method_name.split("(")[
                        0
                    ]  # get the function name only
                    array_length_type = 0

                    method_type, array_length_type = remove_brackets(
                        method_type, array_length_type
                    )
                    method_type = remap_file_path(
                        method_type
                    )  # remap the dots to / and add the L ; or remap to a primitives character
                    method_type = (
                        "L" + file_name[method_type] + ";"
                        if method_type in file_name
                        else method_type
                    )  # get the obfuscated name of the class
                    if (
                        "." in method_type
                    ):  # if the class is already packaged then change the name that the obfuscated gave
                        method_type = "/".join(method_type.split("."))
                    for i in range(
                        array_length_type
                    ):  # restore the array brackets upfront
                        if method_type[-1] == ";":
                    ...
                    if variables != "":  # if there is variables
                        ...
                        for i in range(
                            len(variables)
                        ):  # remove the array brackets for each variable
                            variables[i], array_length_variables[i] = remove_brackets(
                                variables[i], array_length_variables[i]
                            )
                        variables = [
                            remap_file_path(variable) for variable in variables
                        ]  # remap the dots to / and add the L ; or remap to a primitives character
                        variables = [
                            "L" + file_name[variable] + ";"
                        ...
                        variables = [
                            ...
                            for variable in variables
                        ]  # if the class is already packaged then change the obfuscated name
                        for i in range(
                            len(variables)
                        ):  # restore the array brackets upfront for each variable
                ...
                obf_name = obf_name.split(":")[0]
                outputFile.write(
                    remap_file_path(obf_name)[1:-1]
                    + " "
                    + remap_file_path(deobf_name)[1:-1]
                    + "\n"
                )
    if not quiet:
        print("Done !")
...
def make_paths(version: str, side: SideType, removal_bool, force, forceno) -> str:
    ...
    if path.is_file() and removal_bool:
        if force:
            ...
            aw = (
                input(
                    f"versions/{version}/{side}.jar already exists, wipe it (w) or ignore (i) ? "
                )
                or "i"
            )
            path = PATH_TO_ROOT_DIR / "versions" / version
...
def delete_dependencies(version: str, side: SideType) -> None:
    ...
    for _dir in [join(path, "com"), path]:
        for f in os.listdir(_dir):
            if os.path.isdir(join(_dir, f)) and split(f)[-1] not in [
                "net",
                "assets",
                "data",
                "mojang",
...
def main():
    ...
    parser.add_argument(
        ...
        default=True,
        required="--nauto" in sys.argv or "-na" in sys.argv,
        help=f"Remap the jar (only if auto off)",
    )
    parser.add_argument(
        "--delete_dep",
        "-dd",
    ...
    parser.add_argument(
        ...
        default=True,
        required="--nauto" in sys.argv or "-na" in sys.argv,
        help=f"Decompile (only if auto off)",
    )
    parser.add_argument(
        "--quiet",
        "-q",
    ...
    parser.add_argument(
        ...
        type=str,
        dest="lib_dir",
        help="Directory containing lib/*.jar files (cfr, fernflower, SpecialSource)",
    )
    parser.add_argument(
        "--no-remap",
        dest="no_remap",
        action="store_true",
        default=False,
        help="Skip remapping step (for unobfuscated dev snapshots)",
    )
    use_flags = False
    ...
    if use_flags:
        ...
        removal_bool = input("Do you want to clean up old runs? (y/N): ") in [
            "y",
            "yes",
        ]
    if use_flags:
        ...
        decompiler = input(
            "Please input your decompiler choice: Fernflower or CFR (CFR/f): "
        )
    decompiler = (
        decompiler.lower()
        if decompiler.lower() in ["fernflower", "cfr", "f"]
        else "cfr"
    )
    if use_flags:
        ...
        if version is None:
            raise ValueError(
                'You must provide a version with --mcversion <version, "latest", or "snap">'
            )
    else:
        version = (
            input(
                f"Please input a valid version starting from 19w36a (snapshot) or 1.14.4 (releases).\n"
                + f"Use 'snap' for the latest snapshot ({snapshot}) or 'latest' for the latest version ({latest}) :"
            )
            or latest
        )
    ...
    decompiled_version = make_paths(
        version, side, removal_bool, args.force, args.forceno
    )
    ...
    if r:
        ...
        has_mappings = get_mappings(version, side, args.quiet)

        if has_mappings and not args.no_remap:
            convert_mappings(version, side, args.quiet)
            get_version_jar(version, side, args.quiet)
            remap(version, side, args.quiet, lib_dir)
        else:
            if not args.quiet and not has_mappings:
                print(f"Skipping remapping for unobfuscated version {version}")
            get_version_jar(version, side, args.quiet)
            jar_path = PATH_TO_ROOT_DIR / "versions" / version / f"{side}.jar"
            temp_jar_path = PATH_TO_ROOT_DIR / "src" / f"{version}-{side}-temp.jar"
            shutil.copy(jar_path, temp_jar_path)
            if not args.quiet:
                print(f"Copied {jar_path} to {temp_jar_path}")

        if decompiler.lower() == "cfr":
            decompile_cfr(decompiled_version, version, side, args.quiet, lib_dir)
        else:
            decompile_fern_flower(
                decompiled_version, version, side, args.quiet, args.force, lib_dir
            )
        if not args.quiet:
            print("===FINISHED===")
